...
async function checkDatabase() {
  ...
  const { error } = await supabase.from('bookings').select('*').limit(1)
  if (error) {
    console.error('BOOKINGS_CHECK_ERROR:', error.code, error.message)
  } else {
    console.log('RESULT: BOOKINGS_TABLE_EXISTS')
  }
}
